...
*******************************************************************************/
void  
user_plug_set_status2(uint16 status)
{
    if (status != plug_param.status) {

        printf("status input! %u\n", status);

        plug_param.status = status;
     /*   //to do 设置每一位
        uint16 onBits = getLastybits(status,PLUG_NUM);
        uint16 offBits = getLastybitsReverse(status,PLUG_NUM);
        uint16 allLights =  getYbits1Value(PLUG_NUM);

        printf("set plug on,set_mask=%,clear_mask=%,enable_mask=%\n",onBits,offBits,allLights);*/
        //gpio_output_conf(onBits,offBits,allLights,0);


        uint8 key0 = getbit(plug_param.status,0);
        uint8 key1 = getbit(plug_param.status,1);

        printf("key0 = %u,key1 = %u\n", key0,key1);

        PLUG_STATUS_OUTPUT(PLUG_RELAY_LED_IO_NUM, key0);
        PLUG_STATUS_OUTPUT(PLUG_RELAY_LED_IO_NUM1, key1);
    }
}
...
LOCAL void plug_key_pressed(void){
	  spi_flash_erase_sector(PRIV_PARAM_START_SEC + PRIV_PARAM_SAVE);
	    spi_flash_write((PRIV_PARAM_START_SEC + PRIV_PARAM_SAVE) * SPI_FLASH_SEC_SIZE,
	                (uint32 *)&plug_param, sizeof(struct plug_saved_param));


	/*	char *json = get_datapoints();
		send_datastreams_to_queue(json);	//publish topic
...
{
	printf("key0 is short pressed,present plug_param.status =%u\n ",plug_param.status);
//	printf("key0 reverserd=%u,status=%u\n",reversebit(plug_param.status,0),plug_param.status);
/*	plug_param.status = reversebit(plug_param.status,0);
	printf("key0 is short pressed,after reverse plug_param.status =%u\n ",plug_param.status);*/

    user_plug_set_status2(reversebit(plug_param.status,0));
...
*******************************************************************************/
LOCAL void
user_plug_short_press2(void)
{
	printf("key1 is short pressed,present plug_param.status =%u\n ",plug_param.status);
	user_plug_set_status2(reversebit(plug_param.status,1));
	plug_key_pressed();
...
user_plug_init2(void)
{
    ...
    single_key[1] = key_init_single(PLUG_KEY_0_IO_NUM1, PLUG_KEY_0_IO_MUX1, PLUG_KEY_0_IO_FUNC1,
                                    NULL, user_plug_short_press2);
    ...
    PIN_FUNC_SELECT(PLUG_RELAY_LED_IO_MUX1, PLUG_RELAY_LED_IO_FUNC1);

    // default to be off, for safety.
    if (plug_param.status == 0xFFFF) {
        plug_param.status = 0;
    }

    PLUG_STATUS_OUTPUT(PLUG_RELAY_LED_IO_NUM, plug_param.status&0x01);
    PLUG_STATUS_OUTPUT(PLUG_RELAY_LED_IO_NUM1, plug_param.status&0x10);
//    PLUG_STATUS_OUTPUT(PLUG_RELAY_LED_IO_NUM, 1);
//    PLUG_STATUS_OUTPUT(PLUG_RELAY_LED_IO_NUM1, 1);
}
#endif
